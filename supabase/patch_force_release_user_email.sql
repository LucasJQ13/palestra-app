-- Liberacion forzada y verificable de email de usuario.
-- Ejecutar en Supabase SQL Editor si "Eliminar y liberar mail" no libera Auth/Profile.

create or replace function public.admin_force_release_user_email(
  p_email text,
  p_reason text default 'Liberacion forzada de correo desde panel administrador'
)
returns table (
  step text,
  detail text,
...
begin
  if not public.current_user_is_administrator() then
    raise exception 'Solo Administrador puede liberar correos';
  end if;
  ...
  if array_length(ids, 1) is null then
    step := 'resultado';
    detail := 'No hay usuario activo en Auth/Identities para ese correo. Si diagnostico solo muestra backup, el correo ya esta libre.';
    affected_count := 0;
    return next;
    return;
  end if;
  ...
  foreach target_id in array ids loop
    ...
    values (
      ...
      '{}'::jsonb,
      auth.uid(),
      coalesce(p_reason, 'Liberacion forzada de correo desde panel administrador')
    )
    ...
    -- Primero nullear responsables en comunidades para evitar FK.
    update public.communities
    set animator_profile_id = null
    where animator_profile_id = target_id;
    get diagnostics affected = row_count;
    if affected > 0 then
      step := 'cleanup';
      detail := 'communities.animator_profile_id';
    ...
    where coordinator_profile_id = target_id;
    get diagnostics affected = row_count;
    if affected > 0 then
      step := 'cleanup';
      detail := 'communities.coordinator_profile_id';
      affected_count := affected;
      return next;
    end if;

    update public.communities
    set diocesan_vocal_profile_id = null
    where diocesan_vocal_profile_id = target_id;
    get diagnostics affected = row_count;
    if affected > 0 then
      step := 'cleanup';
      detail := 'communities.diocesan_vocal_profile_id';
      affected_count := affected;
      return next;
    end if;

    -- Borrar o nullear relaciones conocidas del usuario.
    ...
    if affected > 0 then
      step := 'cleanup';
      detail := 'profile_role_relationships';
      affected_count := affected;
      return next;
    end if;
    ...
    if affected > 0 then
      step := 'cleanup';
      detail := 'community_contact_messages.sender_id';
      affected_count := affected;
      return next;
    end if;
    ...
    if affected > 0 then
      step := 'cleanup';
      detail := 'audit_logs.actor_id';
      affected_count := affected;
      return next;
    end if;

    -- Borrar otras filas publicas que pertenecen al usuario y no son criticas.
    for ref in
    ...
    loop
      ...
      if affected > 0 then
        step := 'cleanup';
        detail := ref.table_schema || '.' || ref.table_name || '.' || ref.column_name;
        affected_count := affected;
        return next;
      end if;
    end loop;

    -- Limpiar tablas auth asociadas antes de auth.users.
    ...
    loop
      ...
      if affected > 0 then
        step := 'auth';
        detail := 'auth.' || ref.table_name || '.' || ref.column_name;
        affected_count := affected;
        return next;
      end if;
    end loop;
  ...
  detail := case
    ...
    then 'No se pudo liberar completamente: aun queda Auth/Identity activo. Revisar filas de verificacion.'
...
grant execute on function public.admin_force_release_user_email(text, text) to authenticated;
