...
alter table public.profiles
  drop constraint if exists profiles_personal_pm_type_check,
  add constraint profiles_personal_pm_type_check
    check (personal_pm_type is null or personal_pm_type in ('pmm', 'pmf'));

alter table public.profiles
  drop constraint if exists profiles_personal_pm_number_check,
  add constraint profiles_personal_pm_number_check
    check (personal_pm_number is null or personal_pm_number > 0);

update public.profiles
set personal_pm_motto = coalesce(personal_pm_motto, pm_motto)
where pm_motto is not null
  and personal_pm_motto is null;
...
begin
  ...
  values (auth.uid(), 'update_my_profile_details_v2', jsonb_build_object('perseverance_start_year', p_perseverance_start_year, 'personal_pm_type', p_personal_pm_type));
end;
$$;
...
begin
  ...
  set
    ...
    personal_pm_type = case
      when public.role_rank(target_role) >= public.role_rank('sedimentador'::public.user_role) and p_personal_pm_type in ('pmm', 'pmf') then p_personal_pm_type
      else null
    end,
    personal_pm_number = case
      when public.role_rank(target_role) >= public.role_rank('sedimentador'::public.user_role) then p_personal_pm_number
      else null
    end,
    personal_pm_province_id = case
      when public.role_rank(target_role) >= public.role_rank('sedimentador'::public.user_role) then selected_pm_province_id
      else null
    end,
    personal_pm_motto = case
      when public.role_rank(target_role) >= public.role_rank('sedimentador'::public.user_role) then nullif(trim(coalesce(p_personal_pm_motto, '')), '')
      else null
    end,
    pm_motto = case
      when public.role_rank(target_role) >= public.role_rank('sedimentador'::public.user_role) then nullif(trim(coalesce(p_personal_pm_motto, '')), '')
  ...
  insert into public.audit_logs (actor_id, action, metadata)
  values (auth.uid(), 'admin_update_profile_details_v2', jsonb_build_object('profile_id', p_profile_id, 'perseverance_start_year', p_perseverance_start_year, 'personal_pm_type', p_personal_pm_type));
end;
$$;
...
as $$
  ...
  order by
    case when auth.users.email_confirmed_at is null then 0 else 1 end,
